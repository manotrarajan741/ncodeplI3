...
@Component({
  selector: 'page-mostsixes',
  templateUrl: 'mostsixes.html',
})
export class MostsixesPage {
...
mostsixes:any;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad MostsixesPage');
	this.getmostsixes()
  }
			...
			data => {
				 ...
				 if(this.resultData.flag == true)
				 {
					 if(this.resultData.data.length > 0){
						 this.mostsixes = this.resultData.data;
						 this.notfound=0;
					   }  	 
					 else{
						this.notfound=1;
